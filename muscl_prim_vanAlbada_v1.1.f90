...
! distance between interface and cell center
a1(:)=2.0*a1(:)/dabs(r1-r2)
a2(:)=2.0*a2(:)/dabs(r3-r2)
a3(:)=2.0*a3(:)/dabs(r3-r2)
a4(:)=2.0*a4(:)/dabs(r4-r3)
...
do j = 1,ndim+2
   ...
   a11(j) =s1*(1.d0-s1*iphi)*a1(j)*r1
   a12(j) =s1*(1.d0+s1*iphi)*a2(j)*r2
   a21(j) =s2*(1.d0-s2*iphi)*a4(j)*r3
   a22(j) =s2*(1.d0+s2*iphi)*a3(j)*r4
enddo
